...
def color_channels_split(image_path):
...
def fixed_thresholding(threshold, normalized_image, image):
    ...
    # image = image[0:500, :]
    #cv2.waitKey(0)
    #cv2.imwrite("processata.png", im_resized)
    #cv2.destroyAllWindows()
    cloudiness = round((count/(np.shape(image)[0]*np.shape(image)[1]))*100,2)
    print(f"Cloudiness percentage: {cloudiness} %")
...
def processing(path):
    ...
    blue, red = color_channels_split(path)
    ...
    if std > 0.03:  # vaue taken from the article
        cloudiness = MCE_thresholding(normalized_image, image)  # bimodal image -> apply MCE thresolding
    else:
        cloudiness = fixed_thresholding(0.03, normalized_image,
                                        image)  # unimodal (made up of just one element, either cloud or sky)image -> apply fixed thresholding
    return cloudiness

if __name__ == "__main__":
    #-----IMAGE UPLOAD-----
    path = "./LR.jpg"
    cloudiness = processing(path)
